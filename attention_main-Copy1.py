...
def prepare_splitted_dataset(partition, labels):
    ...
    pickle.dump(test_index_list, open(setting.run_dir+os.sep+"test_index_list", "wb+"))
    ...
    pickle.dump(all_index_list, open(setting.run_dir+os.sep+"all_index_list", "wb+"))
...
def run():
    ...
    for train_index, test_index, test_index_2, evaluation_index, evaluation_index_2 in split_func(fold='fold', test_fold = setting.cvn):
        ...
        for epoch in range(setting.n_epochs):
            ...
            with torch.set_grad_enabled(False):
                ...
                if best_cv_pearson_score < val_pearson:
                    ...
                    torch.save(best_drug_model, setting.run_dir+os.sep+"model"+os.sep+"model_cv_.pt")
    ...
    with torch.set_grad_enabled(False):
        ...
        for (cur_local_batch, cur_smiles_a, cur_smiles_b), cur_local_labels in test_iter:
            ...
            for i, test_combination in enumerate(test_index_list[cur_test_start_index: cur_test_stop_index]):
                if not path.exists(setting.run_dir+os.sep+"test_" + setting.catoutput_output_type + "_datas"):
                    mkdir(setting.run_dir+os.sep+"test_" + setting.catoutput_output_type + "_datas")
                save(catoutput.narrow_copy(0, i, 1), path.join(setting.run_dir+os.sep+"test_" + setting.catoutput_output_type + "_datas",
        ...
        if not path.exists(setting.run_dir+os.sep+'prediction'):
            mkdir(setting.run_dir+os.sep+'prediction')
        save(np.concatenate((np.array(test_index_list[:sample_size]).reshape(-1,1), mean_prediction.reshape(-1, 1), mean_y.reshape(-1, 1)), axis=1),
             setting.run_dir+os.sep+"prediction"+os.sep+"prediction_" + setting.catoutput_output_type + "_testing")
